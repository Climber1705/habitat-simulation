...
public class FieldStats {
    ...
    */
    ...
    /**
     * Retrieves the immune details an animal in the field.
     *
     * @param field: The field to analyze.
     * @return A string containing the descriptions of a species and their disease count.
     */
    public String getSpeciesImmune(Field field, String species) {
        if (!countsValid) {
            generateCounts(field);
        }
        StringBuilder buffer = new StringBuilder();
    ...
    */
    public void incrementCountAnimal(Animal animalClass) {
        ...
        if(animalClass.disease.isInfected()) {
            count.incrementDisease();
        }
        if(animalClass.disease.isImmune()) {
            count.incrementImmune();
        }
        count.increment();
    }
